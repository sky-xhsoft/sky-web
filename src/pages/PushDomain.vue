<template>
  <div class="push-domain-page">
    ...
    <a-modal
      ...
      :title="currentStep === 1 ? '添加推流域名' : 'CNAME配置'"
      :width="700"
      :footer="false"
...
// 过滤后的域名列表（只显示推流域名 type=0）
const filteredDomains = computed(() => {
  let result = domains.value.filter(d => d.type === 0)
  ...
  try {
    ...
    if (result && result.data) {
      ...
      domains.value = rawDomains.map((d: any) => ({
        ...
        cname: d.TargetDomain || '',
        region: d.Region || '全球加速',
        cnameConfigured: d.BCName || 0
      }))
    } else {
      domains.value = []
    }
